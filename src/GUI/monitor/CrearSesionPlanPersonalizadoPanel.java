...
import GUI.listeners.monitor.ControlBackSesionPlanPersonalizado;
import GUI.listeners.monitor.ControlBotonCrearSesionPlan;
...
/**
 * Controlador que lleva al monitor a la vista para la creacion de una sesion de un plan personalizado
 * 
 * @author Alvaro Casasus, David Palomares, Laura de la Cruz
 *
 */
public class CrearSesionPlanPersonalizadoPanel extends JPanel{
	private JComboBox planesCombo;
	...
	/**
	 * Constructor de la clase CrearSesionPlanPersonalizadoPanel
	 * 
	 * inicializa los componentes de la vista
	 */
	public CrearSesionPlanPersonalizadoPanel() {
		...
		JLabel nombrePanel = new JLabel("Creación sesion plan personalizado");
		...
		String ejActividades[]= {"No hay planes personalizados disponibles"};
		JLabel actividad = new JLabel("Plan personalizado: ");
		planesCombo = new JComboBox(ejActividades);
		planesCombo.setSelectedIndex(0);
		...
		layout.putConstraint(SpringLayout.WEST, planesCombo, 0, SpringLayout.WEST, salaCombo);
		layout.putConstraint(SpringLayout.NORTH, planesCombo, 25, SpringLayout.NORTH, salaCombo);
		...
		layout.putConstraint(SpringLayout.NORTH, crear, 50, SpringLayout.NORTH, planesCombo);
		...
		this.add(salaCombo);
		this.add(planesCombo);
		this.add(back);
		this.add(crear);
	}
	
	/**
	 * Setter que establece la lista de planes personalizados 
	 * 
	 * @param planesPersonalizados lista de planes personalizados actualizada
	 */
	public void setPlanes(List<PlanPersonalizado> planesPersonalizados) {

		if(planesPersonalizados.size()==0){
			return;
		}
		this.remove(planesCombo);
		this.planesCombo.removeAllItems();
		
		for(PlanPersonalizado pp: planesPersonalizados){
			planesCombo.addItem(pp.getNombre());
		}
	 ...
	 */
	public void setControlBackCrearSesionActividades(ControlBackSesionPlanPersonalizado control) {
	 ...
	 */
	public void setControlCrearSesionPlan(ControlBotonCrearSesionPlan control){
	...
	public String getPlan(){
		return this.planesCombo.getSelectedItem().toString();
	}
}
